### 1. Email Verification with Resend (Your Idea)
...
### 2. Crucial Security Fixes (Portfolio Must-Haves)
...
- **Cloudflare Turnstile (CAPTCHA):** Integrating Turnstile on the `/register` and `/login` forms prevents bot registrations and automated attacks. It's a Cloudflare-native, privacy-preserving alternative to reCAPTCHA.

### 3. Core Identity Features

- **Password Reset Flow:** Add a "Forgot Password" link. This will use the exact same logic as Email Verification: generate a token, store in KV, email via Resend, and create a `/reset-password?token=...` page.
