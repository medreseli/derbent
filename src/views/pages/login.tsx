	...
	return (
		<>
			<p className="mb-8 text-center text-sm text-zinc-600">
				Sign in to continue to <span className="font-semibold text-zinc-900">{appName}</span>
			</p>
			...
			{githubClientId && (
				<>
					<a
						href={`/auth/github?${qs}`}
						className="btn btn-secondary mb-6 flex w-full items-center justify-center gap-3 bg-white text-zinc-900 ring-1 ring-inset ring-zinc-300 hover:bg-zinc-50"
					>
			...
			<form method="post" action={`/login?${qs}`} className="space-y-6">
				...
				<div>
					...
					<input type="password" name="password" required placeholder="••••••••" className="form-input mt-2" />
				</div>
